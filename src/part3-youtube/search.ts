...
/**
 * PART 3 — Step-settings UI on the real YouTube `search` action (26 props!).
 * Vendored 1:1 from packages/pieces/community/youtube.
 *
 * Today every one of these props renders as one flat wall of fields.
 * YOUR TASKS (metadata only — do not change names, types, or run()):
 *  1. Essential/Advanced split: add `advanced: true` to every prop that does
 *     NOT belong in the short essential form a first-time user sees.
 *     A prop is Advanced ONLY if explicitly flagged.
 *  2. Add `propertyGroups` to the action, grouping related props
 *     (display: 'section' recommended). Every prop you reference must exist,
 *     and no prop may appear in two groups.
 *  3. Polish: add `placeholder` where an example would help (e.g. regionCode),
 *     and `width: 'half'` where two short fields belong side by side.
 *  4. Write 5-10 lines in HOW-I-WORKED.md: which props you kept essential & why.
 */
