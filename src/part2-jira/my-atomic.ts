...
export const coverageMap: CoverageMapEntry[] = [
  {
    ...
    rationale: 'AI agents frequently receive a person name or email and need to resolve it to an exact Atlassian accountId before assigning issues.',
  },
  {
    kind: 'add',
    name: 'list_transitions_for_issue',
    archetype: 'missing-verb',
    vendorEndpoint: 'GET /rest/api/3/issue/{issueIdOrKey}/transitions',
    rationale: 'Agents cannot reliably transition an issue without first discovering available workflow transitions and their transition IDs.',
  },
  {
    kind: 'skip',
    name: 'markdown_to_jira_format',
    rationale: 'Local helper utility with no API call; agents can generate ADF or wiki markup directly in prompt context.',
  },
];

export const myAtomic = createAction({
  name: 'find_user_by_query',
  displayName: 'Find User by Query',
  description: 'Search for Jira users by name, email address, or username to resolve their account ID.',
  audience: 'ai',
  classification: 'SEARCH',
  aiMetadata: {
    description:
      'Finds Jira users matching a name or email string and returns their account IDs and display names. Use this before assign_issue or create_issue when only a person name is provided. Read-only and idempotent.',
    idempotent: true,
  },
  props: {
  ...
  async run(context) {
    ...
    const response = await httpClient.sendRequest<Array<{ accountId: string; displayName: string; emailAddress?: string; active: boolean }>>({
